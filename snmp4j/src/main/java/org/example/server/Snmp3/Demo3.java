package org.example.server.Snmp3;
    ...
    public Demo3(int version) {
            try {
                ...
                if (version == SnmpConstants.version3) {
                    ...
                    usm = new USM(instance,new OctetString("xxx-xxx"), 0);
                    usm1 = new USM(instance,new OctetString("xxx-xx1"), 0);
        ...
        public void sendMessage(Boolean syn, final Boolean bro, PDU pdu, String addr)
            ...
            if (version == SnmpConstants.version3) {
                // 添加用户
                snmp.getUSM().addUser(new OctetString("xxx"),
                ...
                ((UserTarget) target).setSecurityName(new OctetString("user1"));

                byte[] authoritativeEngineID = ((UserTarget) target).getAuthoritativeEngineID();
                OctetString securityName = ((UserTarget) target).getSecurityName();
        ...
        public static void main(String[] args) {
            ...
            Demo3 manager = new Demo3(SnmpConstants.version3);
            // 构造报文
            //PDU pdu = new PDU();
            ...
            ((ScopedPDU) pdu).setMaxRepetitions(2);
            ((ScopedPDU) pdu).setNonRepeaters(1);
            try {
                // 发送消息 其中最后一个是想要发送的目标地址
                //manager.sendMessage(false, true, pdu, "udp:192.168.1.229/161");//192.168.1.229 Linux服务器
                manager.sendMessage(false, true, pdu, "udp:192.168.2.45/161");//192.168.1.233 WinServer2008服务器
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
